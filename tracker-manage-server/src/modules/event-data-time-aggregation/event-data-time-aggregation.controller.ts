...
import {
    EventDataTimeAggregationTrendReqDto
} from "@/modules/event-data-time-aggregation/event-data-time-aggregation.dto";
...
export class EventDataTimeAggregationController {
    ...
    async getEventDataAggregateTrend(@Query() params: EventDataTimeAggregationTrendReqDto) {
        ...
        const aggregateEventData = await this.eventDataTimeAggregationService.aggregateEventData(eventId,  startTimestamp,endTimestamp)
        const aggregateEventTrendData = await this.eventDataTimeAggregationService.getMinuteEventTrendData(eventId, startTimestamp,endTimestamp)
